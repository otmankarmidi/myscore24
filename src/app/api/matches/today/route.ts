...
function mapEspnToMatches(rawList: any[]): Match[] {
  ...
  try {
    ...
    if (rawEspnMatches.length === 0) {
      const activeRoundResults = await Promise.allSettled(
        leagues.map(code => espnPublicProvider.fetchMatchesForLeague(code))
      )
      activeRoundResults.forEach(res => {
        if (res.status === 'fulfilled' && Array.isArray(res.value)) {
          rawEspnMatches.push(...res.value)
        }
      })
      if (rawEspnMatches.length > 0) {
        isRoundFallback = true
      }
    }
    ...
    return NextResponse.json({
      data: matches,
      source: isRoundFallback ? 'MyScore24 Active Matchday Feed (ESPN)' : 'MyScore24 Real Date Live Feed (ESPN)',
      date: dateParam,
      count: matches.length,
      lastUpdated: new Date().toISOString()
    })
  } catch (error: any) {
    console.error('ESPN matches fetch failed, using fallback:', error)
    return NextResponse.json({
      data: mockMatches,
      source: 'MyScore24 Resilient Feed',
      date: dateParam,
      count: mockMatches.length,
      lastUpdated: new Date().toISOString(),
      error: error?.message || 'Remote feed offline'
    })
  }
}
