...
export async function GET(
  ...
  try {
    ...
    // 1. Primary Source: Check API-Football if numeric or valid fixture ID
    if (apiFootballProvider.hasValidApiKey() && !isNaN(Number(cleanId))) {
      try {
        ...
        console.warn(`[API /api/matches/${id}] API-Football details query failed, falling back to ESPN:`, apifbErr)
      }
    }
    ...
    try {
      summary = await espnPublicProvider.fetchMatchSummary(cleanId)
    } catch (espnErr) {
      console.warn(`[API /api/matches/${id}] ESPN summary query failed:`, espnErr)
    }
    ...
    // 3. Tertiary Fallback Source: Mock matches
